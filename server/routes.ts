...
DO NOT suggest: Cap_OffCenter, CapEdge_BR (bright variation), CapEdge_DK (dark variation), or any other position variations.`;
      ...
      if (targetShotCount) {
        ...
        if (targetShotCount === 1) {
          ...
          shotCountInstruction += `. CRITICAL: You are choosing THE SINGLE BEST shot to represent this mic/speaker combination${genre ? ` for ${genre}` : ''}. Include a "selectionRationale" field with a detailed explanation (2-3 sentences) of: 1) Why this specific position captures the essential character of this mic+speaker pairing, 2) Why this distance is optimal over closer/farther options, 3) What tonal qualities make this THE definitive shot a producer would choose if they could only have one`;
...
DO NOT suggest: Cap_OffCenter, CapEdge_BR (bright variation), CapEdge_DK (dark variation), or any other position variations.`;
      ...
      if (targetShotCount) {
        ...
        if (targetShotCount === 1) {
          includeSelectionRationale = true;
          shotCountInstruction += `. CRITICAL: You are choosing THE SINGLE BEST mic/position/distance combination to represent this speaker${genre ? ` for ${genre}` : ''}. Include a "selectionRationale" field with a detailed explanation (2-3 sentences) of: 1) Why this specific mic best captures this speaker's character, 2) Why this position and distance combination is optimal, 3) What makes this THE definitive shot a producer would choose if they could only capture one IR from this speaker`;
        } else if (targetShotCount >= 2 && targetShotCount <= 4) {
          includeSelectionRationale = true;
