...
- **IR Pairing** (`Pairing.tsx`): Role-aware, intent-driven, psychoacoustically informed IR blend analyzer. Supports single-speaker and mixed-speaker modes. Uses 6-band spectral analysis, KB role predictions, learned tonal profiles, and psychoacoustic descriptors. Intent selector (Rhythm/Lead/Clean/Versatile) optimizes role pairing guidelines. Mixed speaker mode determines optimal per-speaker role assignments (e.g., Speaker 1 = Foundation, Speaker 2 = Cut Layer). Copy list includes roles, intent fit, psychoacoustic summaries, and speaker role analysis. Refinement panel auto-opens after favoriting (Heart) or passing (X with no replacement). Mic-family IR swapping dropdowns and ratio adjustment (50/50 through 80/20). Submit Improved saves refined blend as favorite + sends love signal for refined + nope for original. Couldn't Improve sends nope signal with `pairing_unfixable` tag. Skip dismisses refinement.
- **Refinement-Aware Mastery**: Server tracks refinement rates from `pairing_refined`, `pairing_improved`, `pairing_unfixable` feedback tags. High refinement rate (30%+ of recent signals) blocks mastery status and generates course corrections. Refinement stats shown in Learner status display. All signal tags now stored in DB `feedback` column (previously `tags` were silently dropped by Zod validation).
...
- **Usefulness Ranking**: Server-side scoring system (`/api/preferences/usefulness-scores`) that computes per-IR usefulness scores from solo ratings and blend performance. Solo love +10, like +6, meh -3, nope -8. Blend loves +2, likes +1, mehs -1, nopes -3. Versatility bonus for 2+ blend partners. Client-side tier assignment (most likely 1-3, least likely 1-2) scoped to loaded batch. Least likely requires negative solo + zero positive blends. Badges shown on Analyzer batch overview, per-IR cards, and Learner solo evaluation cards. Cache invalidated on signal submit/clear.
- **Blend Favorites**: Persistent localStorage-based favorite blend system (`irscope.blendFavorites`). Heart button on Test AI suggestions and Suggested Pairings saves blends with IR names, ratio, roles, and source. Collapsible Blend Favorites panel in Learner shows all saved blends with copy/remove. Deduplication prevents saving the same blend twice.
- **Test AI Feedback**: Heart/X buttons on each Test AI result. Heart saves as favorite + sends love signal. X on blend results opens refinement panel with mic-family IR swapping dropdowns and ratio adjustment (50/50 through 80/20). Submit Improved saves refined blend as favorite + sends love signal for refined + meh for original. Couldn't Improve sends nope signal with `ai_suggestion_unfixable` tag.
- **Suggested Pairings Refinement**: After voting Love/Like/Meh on a pairing card, a refinement panel auto-opens with mic-family IR swapping and ratio adjustment. Submit Improved saves refined blend to favorites + sends love signal for refined + meh for original. Couldn't Improve sends nope signal with `pairing_unfixable` tag. Skip dismisses refinement. Uses `pairRefine` state keyed by pair key. `assignRank()` auto-populates refinement state with pair's filenames and ratio.
...
- **Amp Designer / Mod Lab**: Translates real-world amp and pedal circuit mods (Jose Arredondo, Snorkler, Cameron, diode swaps, op-amp mods, etc.) into Fractal Audio Axe-FX/FM3 Expert parameters. Knowledge base covers 243 amp models (aligned with AM4 firmware), 66 drive/fuzz models, 15 known mods, and 20+ Expert parameter definitions. AI generates parameter recipes with rationale, starting points, interaction notes, and alternatives.
