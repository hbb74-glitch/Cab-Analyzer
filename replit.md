...
- **A/B Taste Check**: Validation phase before ratio refinement. System picks its top-predicted pairing (favorite) and a low-scoring pairing (decoy), randomly assigns them to A/B positions, and user picks one. If user picks the system's predicted favorite, taste is confirmed and ratio refinement unlocks. If user picks the decoy, system learns its predictions were wrong and returns to more comparison rounds. Max 2 attempts before auto-passing. Skippable.
