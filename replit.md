...
- **Recommendations**: The system offers AI-generated optimal mic/position/distance combinations for speakers and genres. It also refines user-provided IR position lists and suggests speaker pairings based on amplifier descriptions and genre-specific studio techniques. All recommendation endpoints inject learned gear preferences (loved/avoided mics, positions, speakers, combos) into AI prompts with conservative weighting.
...
- **IR Culling**: A batch tool that reduces an IR collection to a target count while maximizing variety (mic types, positions) and quality. Uses spectral similarity analysis and greedy selection algorithm to recommend which IRs to keep vs cut. Gear sentiment from learned preferences applies ±5 point scoring adjustments to favor loved gear and penalize noped gear during culling. Includes **Blend Redundancy Detection**: automatically identifies multi-mic blend IRs (e.g., SM57+R121) and compares them against their individual component single-mic captures in the same batch. Verdicts: "essential" (<78% similar to any component — blend creates unique tone), "adds-value" (78-90% similar — moderate tonal contribution), or "redundant" (>90% similar to a component — blend duplicates what a single mic already provides). Redundant blends receive -8 scoring penalty, adds-value get -3. UI shows color-coded badges and per-component similarity percentages.
...
- **Per-Profile Learning**: Adjustments are computed independently for each profile (Featured, Body) using only signals that matched that profile. Featured signals shift Featured targets, Body signals shift Body targets. This preserves the variety of tones the user likes rather than converging on a single average. Falls back to global adjustments when per-profile data is sparse (<2 signals). Per-profile base targets match the hardcoded profile ideals (Featured: mid=22, presence=34; Body: mid=34, presence=12).
...
- **Iterative Suggested Pairings**: Multi-round refinement system. Each round presents a dynamic number of fresh 50:50 blends (3 for small pools, scaling up to 7 for larger pools based on ~70% of pool size), excluding previously evaluated pairs. IR diversity constraint limits how many times any single IR can appear within a round, forcing the algorithm to spread across different IRs for faster learning. Users rate blends with sentiment (Love/Like/Meh) or Nope, with ties allowed. Submit to refine the learned profile, then get a new round of suggestions informed by updated preferences. Shows round counter and cumulative signal stats. Two exit paths: "Submit & Show More" (keep refining) or "Load Top Pick" (done). Continues until user is satisfied or all pair combinations are exhausted.
...
- **Cross-Cab Pairing**: A dedicated section in the IR Mixer for loading IRs from two different speaker cabinets (Cabinet A and Cabinet B). Computes all cross-cabinet blend permutations, ranks them by tonal profile match, and displays expandable results with per-ratio breakdowns. Users can load any cross-cab blend directly into the main mixer for further analysis. Supports adjustable A/B blend ratios.
