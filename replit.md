...
- **Tonal Feedback Tags**: Contextual quick-tag system on pairing ratings. Love shows validation tags (Perfect/Balanced/Punchy/Warm/Aggressive), Like shows improvement suggestions (More bottom/Less harsh/More bite/Tighter/More air), Meh shows problem tags (Thin/Muddy/Harsh/Dull/Boomy/Fizzy). Tags are optional (one per rating), stored in `preference_signals.feedback`, and feed into the learning algorithm as band-specific nudges. Noped problem tags also generate targeted avoid zones (e.g., 2+ "muddy" nopes create a lowMid avoid zone). Tags map to specific band adjustments: bass/lowMid tags route through mid and ratio dimensions proportionally. Course corrections report active feedback tags.
